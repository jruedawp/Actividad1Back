...
<div class="container">
    ...
    // Recuperamos la serie que vamos a editar
    $idSerie = $_GET['id'];
    $seriesController = new SeriesController();
    $serieObject = $seriesController->getSeriesData($idSerie);
    ?>
</div>
<div class="row">
    <div class="col-12">
    ...
    <div class="col-12">
        <form name="edit_series" action="" method="POST">
            <div class="mb-3">
                ...
                <input id="serieTitle" name="serieTitle" type="text" placeholder="Introduce el título de la serie" class="form-control" required value="<?php if(isset($serieObject)) echo $serieObject->getTitle(); ?>"/>
            </div>
...
if ($sendData) {
    if(
        ...
        $serieEdited = $serieAux->updateSeries(
            $_POST['serieId'],
            $_POST['serieTitle'],
            $_POST['serieDirector'],
            $_POST['seriePlatform'],
...
if (!$sendData) {
    // Mostrar formulario vacío si no se ha enviado nada
} else {
    if ($serieEdited) {
        ?>
        <div class="row">
            <div class="alert alert-success" role="alert">
                Serie editada correctamente.<br><a href="listSeries.php">Volver al listado de series.</a>
            </div>
        </div>
        <?php
    } else {
        ...
        <div class="row">
            <div class="alert alert-danger" role="alert">
                La serie no se ha editado correctamente.<br><a href="editSeries.php">Volver a intentarlo.</a>
            </div>
        </div>
        <?php
    }
